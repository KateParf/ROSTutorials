...
Это ClassLoaderключевой класс для понимания, определенный в class_loader.hpp заголовочном файле :
...
Существует несколько способов создания экземпляра класса. В этом примере мы используем общие указатели. Нам просто нужно вызвать createSharedInstanceс полностью определенным типом класса плагина, в данном случае polygon_plugins::Square.

Важное примечание: polygon_baseпакет, в котором определен этот узел, НЕ зависит от polygon_pluginsкласса. Плагины будут загружены динамически без необходимости объявления какой-либо зависимости. Более того, мы создаем экземпляры классов с жестко закодированными именами плагинов, но вы также можете делать это динамически с параметрами и т. д.
